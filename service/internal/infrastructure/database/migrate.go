...
func createMigrationsTable(ctx context.Context, tx *sqlx.Tx) error {
	...
	_, err = tx.ExecContext(ctx, "LOCK TABLE __migrations IN EXCLUSIVE MODE")
	if err != nil {
...
func listAppliedMigrations(ctx context.Context, tx *sqlx.Tx) ([]string, error) {
	var result []string

	if err := tx.SelectContext(ctx, &result, "SELECT migration_file FROM __migrations ORDER BY sequence"); err != nil {
...
func applyMigration(ctx context.Context, tx *sqlx.Tx, box *packr.Box, file string) error {
	...
	if _, err := tx.ExecContext(ctx, "INSERT INTO __migrations(migration_file) VALUES ($1)", file); err != nil {
